<script lang="ts">
  ...
  let liquidityData = $state<LiquidityData>({
    ...
    auctions: { current: 0, change: 0, level: 40 }
  });
  ...
  const tankPositions: TankPositions = {
    ...
    auctions: { x: 50, y: 250, label: 'Treasury Auctions\n(New Issuance)', width: 110, height: 140 }
  };
  ...
  function generateFlows(data: LiquidityData): void {
    ...
    const relationships = [
      ...
      // Treasury auction flows
      { from: 'auctions', to: 'tga', condition: () => data.auctions.change > 0 && data.tga.change > 0 },
      { from: 'rrp', to: 'auctions', condition: () => data.rrp.change < 0 && data.auctions.change > 0 },
      { from: 'auctions', to: 'rrp', condition: () => data.auctions.change < 0 && data.rrp.change > 0 },
      { from: 'walcl', to: 'auctions', condition: () => data.walcl.change > 0 && data.auctions.change > 0 },
      { from: 'auctions', to: 'soma', condition: () => data.auctions.change < 0 && data.soma.change > 0 }
    ];
    ...
    try {
      ...
      const promises = endpoints.map(async (endpoint) => {
        try {
          ...
          if (endpoint.key === 'auctions') {
            // Treasury auctions have different structure
            latest = data.output[data.output.length - 1];
            ...
            return {
              key: endpoint.key,
              current: parseFloat(latest?.total_outflow) || 0,
              change: parseFloat(change) || 0
            };
          } else {
            // Other endpoints use 'value' field
            latest = data.output[data.output.length - 1];
            change = latest?.pct_change;
  ...
  <div class="max-w-7xl mx-auto">
    ...
    {:else}
      ...
      <div class="bg-slate-900/50 rounded-2xl p-8 border border-slate-800 mb-8 backdrop-blur-sm">
        <svg width="100%" height="600" viewBox="0 0 950 550" class="overflow-visible">
          <defs>
