...
export const GET: RequestHandler = async () => {
    ...
    const url = `https://api.stlouisfed.org/fred/series/observations?series_id=WDTGAL&api_key=${VITE_FRED_API}&file_type=json&observation_start=${thirtyDaysAgo}&observation_end=${today}`;
    const res = await fetch(url)
    if(!res.ok){
        return new Response('Failed to fetch WDTGAL', {status: 500})
    }
