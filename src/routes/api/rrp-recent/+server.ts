...
export const GET: RequestHandler = async () => {
    ...
    const thirtyDaysAgo = new Date(Date.now() - 60 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
    const url = `https://api.stlouisfed.org/fred/series/observations?series_id=RRPONTSYD&api_key=${VITE_FRED_API}&file_type=json&observation_start=${thirtyDaysAgo}&observation_end=${today}`;
    const res = await fetch(url)
    if(!res.ok){
        return new Response('Failed to fetch RRPONTSYD', {status: 500})
    }
